        ...
        private static QMachine<char> CreateMachine(Random random)
        {
            ...
            .WithMutation(new MutationLayerParameter<char>
            {
                ...
                .AddMutation(Mutations.Mutation<char>()
                    .Frequency(0.99)
                    .When(Pixel.WhitePixel & CartesianFilters.MooreFilter<char>(neighbors => neighbors.Count(Filters.EqualsToValue(Pixel.BlackPixel).ApplyTo) == 3))
                    .Into(Pixel.BlackPixel)
                    .Build())
                .AddMutation(Mutations.Mutation<char>()
                    .Frequency(0.99)
                    .When(Pixel.BlackPixel & CartesianFilters.MooreFilter<char>(neighbors => neighbors.Count(Filters.EqualsToValue(Pixel.BlackPixel).ApplyTo) > 3))
                    .Into(Pixel.WhitePixel)
                    .Build())
                .AddMutation(Mutations.Mutation<char>()
                    .Frequency(0.99)
                    .When(Pixel.BlackPixel & CartesianFilters.MooreFilter<char>(neighbors => neighbors.Count(Filters.EqualsToValue(Pixel.BlackPixel).ApplyTo) < 2))
                    .Into(Pixel.WhitePixel)
                    .Build())
                .AddMutation(Mutations.Mutation<char>()
                    .Frequency(0.1)
                    .When(Pixel.WhitePixel)
